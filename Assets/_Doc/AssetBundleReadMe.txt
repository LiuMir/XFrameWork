一、打包（一个或者多个资源打成一个ab）
	...
	1、划分资源粒度（控制ab大小）
		...
		2）：配置文件： 需要打成ab的文件夹根路径、ab输出路径、对应的平台、 ab划分规则、上传的服务器地址（可选）
			...
			特例（内置资源打成一个ab包、内置shader找替代）： 内置资源、shader
	
	2、设置assetbundle名字

		特例：图集设置bundle名字最好与Packing Tag一致（一个图集一个ab包）
...
二、热更（不用重新下载安装包，能够更新资源或者逻辑）
	...
	1、热更准备工作
		...
		1）：资源版本号文件、热更资源列表文件
			...
			热更资源列表文件：相比于基础包（出apk、ipa那时刻的安装包）的差异ab文件列表，每一个ab文件由路径、md5（hash -> 每一个ab包都有自己的hash值[AssetBundleManifest]）值组成
	...
	2、下载热更资源
		...
		2）：下载热更资源列表文件，对比本地热更资源列表文件，找出两文件间的差异。新增、修改则加入下载列表；删除则加入删除列表
			...
			差异对比方式：①、本地记录文件有没有 ②、与本地记录文件的md5(hash)值是否一致 ③、本地记录文件多余项
